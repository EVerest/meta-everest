LICENSE = "Apache-2.0 & ISC & MIT & Python-2.0 & BSD-3-Clause & BSD-2-Clause"
LIC_FILES_CHKSUM = "file://${COMMON_LICENSE_DIR}/Apache-2.0;md5=89aea4e17d99a7cacdbeed46a0096b10 \
                    file://node_modules/@babel/parser/LICENSE;md5=3b324af8e79986f4a5621efa85dd1292 \
                    file://node_modules/@koumoul/vjsf/LICENSE;md5=0cf31bb9e2152bcb0ec2adeefcec6469 \
                    file://node_modules/@koumoul/vjsf/dist/main.js.LICENSE.txt;md5=3cae12c79e2f96eecac06d4302ea88cf \
                    file://node_modules/@koumoul/vjsf/dist/third-party.js.LICENSE.txt;md5=8b15ee38193dcbea54a780ad16c4d219 \
                    file://node_modules/@mdi/font/LICENSE;md5=7f104afa30591ec383c5140124c88aef \
                    file://node_modules/@mdi/js/LICENSE;md5=7f104afa30591ec383c5140124c88aef \
                    file://node_modules/@vue/compiler-sfc/LICENSE;md5=9fcdf292094b4bb62e6ae00235fcc719 \
                    file://node_modules/ajv-formats/LICENSE;md5=b070047241b584db26163b1dca5206e0 \
                    file://node_modules/ajv-i18n/LICENSE;md5=715855234c57d97032971adfc0608a07 \
                    file://node_modules/ajv/LICENSE;md5=5ed8db7ae36f56c8a5cfc218c41ac926 \
                    file://node_modules/argparse/LICENSE;md5=203a6dbc802ee896020a47161e759642 \
                    file://node_modules/core-js/LICENSE;md5=bc0d549e3083e081fa05a7bb415f3a36 \
                    file://node_modules/csstype/LICENSE;md5=e71640b3d80aa47c69967e588c744bfb \
                    file://node_modules/debounce-promise/LICENSE;md5=fb7853fc1e4714184d50288c4ea59f08 \
                    file://node_modules/debounce/LICENSE;md5=3ff25a3a15c428c20fe547f4fa5e76d3 \
                    file://node_modules/debug/LICENSE;md5=d85a365580888e9ee0a01fb53e8e9bf0 \
                    file://node_modules/entities/LICENSE;md5=d5b9cb3bc7f6ffd7bea8661f30447c11 \
                    file://node_modules/expr-eval/LICENSE.txt;md5=d04332de3a8a7c4efc44a693ca629407 \
                    file://node_modules/fast-copy/LICENSE;md5=bec9eb9e9b93e780c6e98e264d188b34 \
                    file://node_modules/fast-deep-equal/LICENSE;md5=ea87ade09b9e6da4f2e47904a4ee137b \
                    file://node_modules/fast-equals/LICENSE;md5=77dbf7585d1337fc314a126be9d1d558 \
                    file://node_modules/json-schema-traverse/LICENSE;md5=ea87ade09b9e6da4f2e47904a4ee137b \
                    file://node_modules/just-clone/LICENSE;md5=9a101e543aed27cd8558f6376292442e \
                    file://node_modules/konva/LICENSE;md5=164b7922ad16f823f27788d36136a44b \
                    file://node_modules/linkify-it/LICENSE;md5=e2f6d3a49c2238880c1d728ddc187f7a \
                    file://node_modules/markdown-it/LICENSE;md5=4323c95e1014fb58368f2d4c2b78064b \
                    file://node_modules/match-all/LICENSE;md5=53e80078cd02a6dd9555b612acad2860 \
                    file://node_modules/mdurl/LICENSE;md5=4dd6bce4f4fc62f6f51f603c406c145b \
                    file://node_modules/ms/license.md;md5=fd56fd5f1860961dfa92d313167c37a6 \
                    file://node_modules/nanoid/LICENSE;md5=237c21016e56a1ee4475a39fc00d1504 \
                    file://node_modules/picocolors/LICENSE;md5=8aabd5d21f92335888d962fb6ca980f8 \
                    file://node_modules/postcss/LICENSE;md5=e0ef868fdaaba6859dcbab082c20439b \
                    file://node_modules/property-expr/LICENSE.txt;md5=229f05540d2758193231934e7a56e52c \
                    file://node_modules/punycode/LICENSE-MIT.txt;md5=ee9bd8b835cfcd512dd644540dd96987 \
                    file://node_modules/register-service-worker/LICENSE;md5=9fcdf292094b4bb62e6ae00235fcc719 \
                    file://node_modules/require-from-string/license;md5=be72c3ad86c1c4e9578a1945b082b17d \
                    file://node_modules/roboto-fontface/LICENSE;md5=221f8a27f8588616f09cdec213993112 \
                    file://node_modules/sortablejs/LICENSE;md5=de4aebaf7b7d0d4c8906e079c511b3d0 \
                    file://node_modules/source-map-js/LICENSE;md5=b1ca6dbc0075d56cbd9931a75566cd44 \
                    file://node_modules/source-map/LICENSE;md5=b1ca6dbc0075d56cbd9931a75566cd44 \
                    file://node_modules/uc.micro/LICENSE.txt;md5=ee9bd8b835cfcd512dd644540dd96987 \
                    file://node_modules/uri-js/LICENSE;md5=3b55dad4a98748003b5b423477713da1 \
                    file://node_modules/vue-class-component/LICENSE;md5=761fb17e53b4b87a0507c26d4bd5bfa4 \
                    file://node_modules/vue-property-decorator/LICENSE;md5=39a26518d5ca1ab24b5c7d7bff548214 \
                    file://node_modules/vue-router/LICENSE;md5=ccd8a7df2a12a2ac7612b56d2af31f87 \
                    file://node_modules/vue/LICENSE;md5=9fcdf292094b4bb62e6ae00235fcc719 \
                    file://node_modules/vuedraggable/LICENSE;md5=5483bbc43f327d3eea14de742d4d0cda \
                    file://node_modules/vuetify/LICENSE.md;md5=61626e12e39ac039026fd688eb9b1b56 \
                    file://node_modules/vuex/LICENSE;md5=761fb17e53b4b87a0507c26d4bd5bfa4"

SRC_URI = "https://github.com/EVerest/everest-admin-panel/releases/download/v0.1.0/everest-admin-panel.tar.gz \
           npmsw://${THISDIR}/${BPN}/npm-shrinkwrap.json \
          "
SRC_URI[sha256sum] = "685d0f58a1b4501667b98d967c9f7b99ce699d02caccef47a05521f37648ef50"

PV = "0.1"

FILES_${PN} += "${datadir}/everest/www/*"

do_install() {
    install -d ${D}/usr/share/everest/www
    cp -a --no-preserve=ownership ${S} ${D}/usr/share/everest/www/
}

INSANE_SKIP_${PN} += "file-rdeps"

LICENSE_${PN} = "Apache-2.0"
LICENSE_${PN}-babel-parser = "MIT"
LICENSE_${PN}-koumoul-vjsf = "MIT"
LICENSE_${PN}-mdi-font = "Apache-2.0 & MIT"
LICENSE_${PN}-mdi-js = "Apache-2.0 & MIT"
LICENSE_${PN}-vue-compiler-sfc = "MIT"
LICENSE_${PN}-ajv = "MIT"
LICENSE_${PN}-ajv-formats = "MIT"
LICENSE_${PN}-ajv-i18n = "MIT"
LICENSE_${PN}-argparse = "Python-2.0"
LICENSE_${PN}-core-js = "MIT"
LICENSE_${PN}-csstype = "MIT"
LICENSE_${PN}-debounce = "MIT"
LICENSE_${PN}-debounce-promise = "MIT"
LICENSE_${PN}-debug = "MIT"
LICENSE_${PN}-entities = "BSD-3-Clause"
LICENSE_${PN}-expr-eval = "MIT"
LICENSE_${PN}-fast-copy = "MIT"
LICENSE_${PN}-fast-deep-equal = "MIT"
LICENSE_${PN}-fast-equals = "MIT"
LICENSE_${PN}-json-schema-traverse = "MIT"
LICENSE_${PN}-just-clone = "MIT"
LICENSE_${PN}-konva = "MIT"
LICENSE_${PN}-linkify-it = "MIT"
LICENSE_${PN}-markdown-it = "MIT"
LICENSE_${PN}-match-all = "MIT"
LICENSE_${PN}-mdurl = "MIT"
LICENSE_${PN}-ms = "MIT"
LICENSE_${PN}-nanoid = "MIT"
LICENSE_${PN}-picocolors = "ISC"
LICENSE_${PN}-postcss = "MIT"
LICENSE_${PN}-property-expr = "MIT"
LICENSE_${PN}-punycode = "MIT"
LICENSE_${PN}-register-service-worker = "MIT"
LICENSE_${PN}-require-from-string = "MIT"
LICENSE_${PN}-roboto-fontface = "Apache-2.0"
LICENSE_${PN}-sortablejs = "MIT"
LICENSE_${PN}-source-map = "BSD-3-Clause"
LICENSE_${PN}-source-map-js = "BSD-3-Clause"
LICENSE_${PN}-ucmicro = "MIT"
LICENSE_${PN}-uri-js = "BSD-2-Clause"
LICENSE_${PN}-vue = "MIT"
LICENSE_${PN}-vue-class-component = "MIT"
LICENSE_${PN}-vue-property-decorator = "MIT"
LICENSE_${PN}-vue-router = "MIT"
LICENSE_${PN}-vuedraggable = "MIT"
LICENSE_${PN}-vuetify = "MIT"
LICENSE_${PN}-vuex = "MIT"
